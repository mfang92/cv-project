...
from train_2016_video import train_model
import sys
...
def main():
    cwd = os.getcwd()
    save_dir = osp.join(cwd, "saved_model_2016")
    print(save_dir)
    data_dir = "/my_vol/tiny" # volume

    print("main, save_dir", save_dir)

    model_name = "video_upsample-1_9-3-5-epoch_20_size_200_patches_100"
    state, val_loss, train_loss = model_run.remote(data_dir, model_ind=20, size_lim=10000, num_epochs=25, batch_size=16, num_workers=4) # total num of patches <= size_lim * num
